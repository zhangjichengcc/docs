...
function themeConf() {
  ...
  const theme =
    ...
    Docsify.dom.find('html').getAttribute('data-theme') ||
    'light';
  // 获取 theme 列表
  const themes = Docsify.dom.findAll('[data-style-theme]');
  let lightCover =
      'background: linear-gradient(to left bottom, hsl(191, 100%, 85%) 0%,hsl(93, 100%, 85%) 100%)',
    darkCover =
      'background: linear-gradient(to left bottom, hsl(0deg 0% 0%) 0%,#3c3c3c 100%);';
